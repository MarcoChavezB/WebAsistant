...
@Component({
  selector: 'app-gradica-velocidad',
  standalone: true,
  imports: [],
  templateUrl: './gradica-velocidad.component.html',
  styleUrl: './gradica-velocidad.component.css'
})
export class GradicaVelocidadComponent implements AfterViewInit {
  ...
  velodata(){
    this.sensorservice.getVelocidadData(this.deviceservice.getStoredIdDevice()).subscribe(
      (data) => {
        if(data.data !== null){
      ...
      (err) => {
        console.error('Error al obtener datos de velocidad:', err);
      }
    )
  }
